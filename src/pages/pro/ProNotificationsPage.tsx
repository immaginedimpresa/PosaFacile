...
export function ProNotificationsPage() {
    ...
    useEffect(() => {
        if (user) {
            loadNotifications(user.id, 'professional')
            loadPreferences()
        }
    }, [user?.id])
    ...
    const handleTogglePreference = async (
        ...
        if (ok) {
            ...
            toast.success(`Preferenza per "${eventType}" salvata!`)
        } else {
    ...
    const filteredNotifications = notifications.filter((item) => {
        ...
        if (categoryFilter === 'jobs' && item.type === 'job_assigned') return true
        if (categoryFilter === 'messages' && item.type === 'message_received') return true
        if (categoryFilter === 'status' && (item.type === 'status_changed' || item.type === 'pro_approved')) return true

        return false
    })

    const totalCount = notifications.length
    const jobsCount = notifications.filter((n) => n.type === 'job_assigned').length
    const messagesCount = notifications.filter((n) => n.type === 'message_received').length
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-stone-200/90 p-5 shadow-xs flex items-center justify-between">
                    <div>
                        ...
                        <div className="text-2xl sm:text-3xl font-black text-emerald-600 mt-1">{jobsCount}</div>
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-stone-200/90 p-5 shadow-xs flex items-center justify-between">
                    <div>
                        ...
                        <div className="text-2xl sm:text-3xl font-black text-blue-600 mt-1">{messagesCount}</div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-stone-200/90 shadow-xs flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {[
                                { id: 'all', label: 'Tutti' },
                                { id: 'jobs', label: 'Cantieri Assegnati' },
                                { id: 'messages', label: 'Messaggi' },
                                { id: 'status', label: 'Stati & Profilo' },
                            ].map((cat) => (
                                <button
                                    key={cat.id}
                                    type="button"
                    ...
                    <div className="space-y-3">
                        {filteredNotifications.length === 0 ? (
                            <div className="bg-white rounded-3xl border border-stone-200/80 p-12 text-center">
                                ...
                                <h3 className="text-base font-bold text-stone-800">Nessun avviso presente</h3>
                                <p className="text-xs text-stone-500 mt-1 max-w-sm mx-auto">
                                    {unreadOnly
                                        ? 'Non hai avvisi in attesa di lettura.'
                                        : 'Tutte le comunicazioni sui tuoi cantieri appariranno qui in tempo reale.'}
                                </p>
                            </div>
                        ) : (
